...
| Task                               | Read first                                                                                   |
| ---------------------------------- | -------------------------------------------------------------------------------------------- |
| Change scoring or readiness floors | `benchmark/v0.4/benchmark.yaml`, `benchmark/v0.4/scoring-policy.md`                          |
| Add or edit a control              | `.agents/skills/benchmark-authoring/SKILL.md`, the matching `benchmark/v0.4/controls/*.yaml` |
| Change evidence collection         | `src/evidence.ts`, `src/schema.ts`                                                           |
| Change reports or CLI behavior     | `src/report.ts`, `src/cli.ts`                                                                |
| Add a vendor adapter               | `benchmark/v0.4/adapter-contract.md`; do not change portable control meaning                 |
...
```bash
npm run check
node dist/cli.js assess tests/fixtures/mature --profile pr-creation --format markdown --attestations tests/fixtures/mature/.agentic/attestations-v0.4.yaml
```
